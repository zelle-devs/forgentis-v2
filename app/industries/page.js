...
// ThirdSection (ResourceSection) — optimized mobile stepping for sequential cards
const getThirdStep = () => {
    if (typeof window === 'undefined') return THIRD_STEP
    const w = window.innerWidth
    if (w <= 480) return 0.005  // Thora sa step barha diya taake lag na ho aur foran response mile
    if (w <= 768) return 0.007
    if (w <= 1024) return 0.010
    return THIRD_STEP
}

const getThirdSplit = () => {
    if (typeof window === 'undefined') return THIRD_SPLIT
    // Mobile par entry split ko bhi kam kiya taake section enter hote hi foonan andar move ho
    return window.innerWidth <= 768 ? 0.08 : THIRD_SPLIT
}

const getThirdInternalSplit = () => {
    if (typeof window === 'undefined') return THIRD_INTERNAL_VERTICAL_SPLIT

    return window.innerWidth <= 768 ? 0.15 : THIRD_INTERNAL_VERTICAL_SPLIT
}
...
//     if (typeof window === 'undefined') return THIRD_STEP
//     const w = window.innerWidth
//     if (w <= 480) return 0.003  // Pehle 0.006 tha, ab mazeed slow kar diya taake ek ek karke smooth aayein
//     if (w <= 768) return 0.004  // Pehle 0.007 tha
...
// const getThirdSplit = () => {
//     if (typeof window === 'undefined') return THIRD_SPLIT
//     return window.innerWidth <= 768 ? 0.15 : THIRD_SPLIT
// }

// const getThirdInternalSplit = () => {
//     if (typeof window === 'undefined') return THIRD_INTERNAL_VERTICAL_SPLIT
//     // Mobile par internal split ko 0.75 se kam karke 0.35 kar diya 
//     // taake vertical intro khatam hote hi foran images / sequential cards ana shuru ho jayein
//     return window.innerWidth <= 768 ? 0.35 : THIRD_INTERNAL_VERTICAL_SPLIT
// }
...
export default function Industries() {
    ...
    const handleScrollStep = (direction, factor) => {
        ...
        if (stageRef.current === STAGE_THIRD) {
            const step = getThirdStep()
            const thirdSplit = getThirdSplit()
            const internalSplit = getThirdInternalSplit()
            ...
            if (direction > 0) {
                ...
                if (scrollProgressRef.current <= thirdSplit) {
                    slideParallax(
                        thirdRef,
                        capabilityRef,
                        scrollProgressRef.current / thirdSplit
                    )
                    ...
                    if (activeProgress <= internalSplit) {
                        const verticalProgress =
                            activeProgress / internalSplit
...
// const getFinalStep = () => {
//     if (typeof window === 'undefined') return FINAL_STEP
//     const w = window.innerWidth
//     if (w <= 480) return 0.012
//     if (w <= 768) return 0.012
//     if (w <= 1024) return 0.011
//     return FINAL_STEP
// }
