...
//   - SingleQuality → 0.025 (Fast & responsive)
...
//   singleQuality  → 0.025  ← Fast scroll
//   sectionThree   → 0.005  ← SLOWED (Capability jaisa)
//   stats          → 0.005  ← SLOWED (Capability jaisa)
//   final          → 0.030  ← extra sensitive
export const SCROLL_STEPS = {
 ...
 singleQuality: { desktop: 0.02,  mobile: 0.02 }, // ← Naya section (Fast)
...
export const RESPONSIVE_STEPS = {
 ...
 singleQuality: {
    desktop: 0.02,   // 👉 Desktop slow & controlled
    tablet:  0.02,   // 👉 Tablet bhi slow & controlled
    mobile:  0.02,   // Mobile speed waisi hi rakhi hai
    small:   0.02,   
  },
}
...
export const SPLITS = {
  capability:    0.2,
  singleQuality: 0.3, // ← Naya split point
  sectionThree:  0.4,
  stats:         0.4,
  final:         0.4,
}
...
export const getSingleQualityStep = () => {
  if (typeof window === 'undefined') return RESPONSIVE_STEPS.singleQuality.desktop
  const w = window.innerWidth
  if (w <= 480) return RESPONSIVE_STEPS.singleQuality.small
  if (w <= 768) return RESPONSIVE_STEPS.singleQuality.mobile
  if (w <= 1024) return RESPONSIVE_STEPS.singleQuality.tablet
  return RESPONSIVE_STEPS.singleQuality.desktop
}
