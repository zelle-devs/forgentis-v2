...
function CapabilitySection({
  ...
  useEffect(() => {
    ...
    // Panel height — mobile pe 58%, desktop pe full
    const getPanelHeight = () => {
      const rawVh = getRawVh()
  ...
  useEffect(() => {
    const applyScroll = () => {
      ...
      POINTS.forEach((point, pIndex) => {
        ...
        if (point.button) {
          const btnSpeed =
            point.button.pos?.speed ?? point.descPos?.speed ?? 0.08
    ...
    const handleCapabilityProgress = (e) => {
      ...
      const panelHeight =
        window.innerWidth <= 768 ? rawVh * 0.58 : rawVh

      vh.current = panelHeight

      const maxScroll = (TOTAL_PANELS - 1) * panelHeight
    ...
    return () =>
      window.removeEventListener('capabilityProgress', handleCapabilityProgress)
  }, [POINTS, TOTAL_PANELS])
...
//                 className="capability-point-title"
//                 style={{
//                   top: isMobile ? '58%' : point.titlePos.top,
//                   left: isMobile ? '6%' : point.titlePos.left,
//                 }}
//               >
//                 <span className={isEven ? 'highlight-white' : 'highlight-blue'}>
...
//                   style={{
//                     position: 'absolute',
//                     top: isMobile ? '84%' : point.button.pos?.top,
//                     left: isMobile ? '6%' : point.button.pos?.left,
