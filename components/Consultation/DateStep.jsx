...
const DateStep = ({
    ...
    { 
      ...
      logo: (
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
          ...
          <text x="7" y="14.5" fontFamily="'Segoe UI', Arial, sans-serif" fontWeight="900" fontSize="7.5" fill="white" textAnchor="middle">T</text>
        </svg>
      )
    },
    { 
  ...
  const meetingModes = [
    {
      ...
      logo: (
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
          <path d="M4 21V8L12 3L20 8V21H14V14H10V21H4Z" fill="#337CC7"/>
        </svg>
      )
    },
    {
      ...
      logo: (
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
          <circle cx="12" cy="12" r="9" fill="#337CC7"/>
  ...
  // Check if calendar is currently on the real-world current month/year
  const now = new Date();
  ...
  const calendarData = useMemo(() => {
    ...
    // Keep grid alignment by pushing blank placeholders instead of prev month numbers
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push({ day: '', dateObj: null, isCurrentMonth: false, isDisabled: true });
    }
        ...
        <div className="separate-calendar-card">
          ...
          <div className="calendar-header">
            <div className="nav-btns">
              {/* Disabled when on the current month */}
              <button 
                className="circle-btn" 
                disabled={isCurrentMonthOrPast}
                style={isCurrentMonthOrPast ? { opacity: 0.3, cursor: 'not-allowed' } : {}}
                onClick={() => {
            ...
            <div className="selectors">
              {/* Month Dropdown: skips past months entirely */}
                ...
                {openDropdown === 'month' && (
                  <div className="pill-dropdown-list" data-dropdown>
                    {Array.from({ length: 12 }, (_, i) => i)
                      .filter(i => {
                        const today = new Date();
                        // If selected year is current year, exclude earlier months
                      ...
                      .map(monthIdx => (
                        ...
                        >
                          {new Date(0, monthIdx).toLocaleString('default', { month: 'long' })}
                        </div>
                      ))}
                  </div>
                )}
                  ...
                  <div className="pill-dropdown-list" data-dropdown>
                    {[2026, 2027, 2028, 2029].map(year => (
                      <div 
                        ...
                        className="pill-option" 
                        onClick={() => { 
                          // Ensure month doesn't remain a past month when selecting current year
                          const thisYear = now.getFullYear();
                          const targetMonth = (year === thisYear && currentMonth.getMonth() < now.getMonth()) 
                            ? now.getMonth() 
                            : currentMonth.getMonth();

                          setCurrentMonth(new Date(year, targetMonth, 1)); 
                          setOpenDropdown(null); 
                        }}
                      >
                        {year}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {calendarData.map((item, idx) => {
                // If it's empty padding for alignment, render blank spacer
                if (!item.isCurrentMonth) {
                  return <div key={`empty-${idx}`} className="day-btn-empty" style={{ pointerEvents: 'none' }} />;
                }
                ...
                return (
                  <button
                    ...
                    className={`day-btn ${isSelected ? 'selected-day' : ""} ${item.isDisabled ? 'disabled-day' : ""}`}
